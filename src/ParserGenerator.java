...
public class ParserGenerator {
	...
	private Map<NonTerminal, Set<Character>> first, follow;
	private NonTerminal start;
	private boolean isLL1;

	private final char EPS = 0;
	private final char EOL = (char) -1;

	private boolean addAllFirst(Set<Character> to, List<GrammarUnit> from,
			int index) {
		if (index == from.size()) {
			return to.add(EPS);
		}
